...
```{r fig1, fig.height = 12, fig.width = 17}
df.eye.baseline <- baseline_correction_pupil(datafile = df.eye.smooth_interp,
                                            pupil_colname = "pup_interp",
                                            baseline_window = c(2500,2600))
...
ggline(quick.summary, x = "Alteration", y = "M", color = "if.Altered",
       add = c("dotplot"),
...
```
```{r}
df.eye.pupilz <- drop_na(df.eye.pupilz)
...
```{r,fig3, fig.height = 7, fig.width = 17}
df.eye.pupilz <- drop_na(df.eye.pupilz)
